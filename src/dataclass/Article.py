...
class Article(article):
    ...
    def insert_or_create_section(
        ...
        for section_name, content_dict in article_dict.items():
            ...
            if current_section_node is None:
                ...
                insert_to_front = parent_node.section_name == (
                    self.root.section_name
                    and current_section_node.section_name == "基本介绍"
                ) or (
                    self.root.section_name
                    and current_section_node.section_name == "引言"
                )
    ...
    def _merge_new_info_to_references(
        self, new_info_list: List, index_to_keep=None
    ) -> Dict[int, int]:
        """
    ...
    def dump_reference_to_file(self, file_path):
        ...
        for url, info in self.reference_["url_to_info"].items():
            self.reference_["url_to_info"][url] = (
                info.to_dict() if hasattr(info, "to_dict") else info
            )
