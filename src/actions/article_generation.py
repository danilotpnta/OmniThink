...
class ArticleGenerationModule:
    ...
    def generate_article(
        ...
        language_style = (
            "{} {}\n".format(
                language_style.get("style", ""), language_style.get("language_type", "")
            )
            if language_style
            else str()
        )
        ...
        article: Article = copy.deepcopy(article_with_outline)
...
class ConvToSection(dspy.Module):
    ...
    def __init__(self, class_name, engine: dspy.LM):
        super().__init__()
    ...
    def forward(
        ...
        section: str,
        collected_info: List,
        language_style: Dict[str, str] = {"language": "English", "style": ""},
    ):
        all_info = ""
        ...
        all_info = ArticleTextProcessing.limit_word_count_preserve_newline(
            all_info, 1500
        )
...
class WriteSection(dspy.Signature):
    ...
    language_style = dspy.InputField(
        prefix="the language style you needs to imitate: ", format=str
    )
...
class WriteSectionAgentEnglish(dspy.Signature):
    ...
    language_style = dspy.InputField(
        prefix="Target writing style (formal English): ", format=str
    )
    output = dspy.OutputField(
...
class WriteSectionAgentEnthusiasticEnglish(dspy.Signature):
    ...
    section = dspy.InputField(prefix="Target section to write: ", format=str)
    language_style = dspy.InputField(
        prefix="Target writing style (enthusiastic English): ", format=str
    )
    output = dspy.OutputField(
