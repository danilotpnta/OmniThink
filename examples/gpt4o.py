...
def main(args):
    ...
    mind_map = MindMap(retriever=rm, gen_concept_lm=lm, depth=args.depth)
    ...
    ag = ArticleGenerationModule(
        retriever=rm, article_gen_lm=lm, retrieve_top_k=3, max_thread_num=10
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--llm", type=str, help="The language model API to use for generating content."
    )
    parser.add_argument(
        "--depth", type=int, default=2, help="The depth of knowledge seeking."
    )
